...
class ClientOffersDetailsPresenter : ClientOffersDetailContract.Presenter {


    private var view: ClientOffersDetailContract.View? = null
    private lateinit var api: AppApi
    private lateinit var retrofit: Retrofit
    private val subscriptions = CompositeDisposable()


    override fun attachView(view: ClientOffersDetailContract.View) {
        this.view = view
    }
    ...
    override fun withDrawOffers(offersId: String) {
        ...
        api.withDrawOffers(offersId)
            ...
            .subscribe(object : NetworkCall<CommonMessageBean>() {
                ...
                override fun onSuccessResponse(dataBean: CommonMessageBean) {
                    view?.hideProgress()
                    view?.onWithdrawOffersSuccessfully(dataBean)
                }

                override fun onFailedResponse(errorBean: Any?) {
                    view?.hideProgress()
                    LogX.E(errorBean.toString())
                    view?.onWithdrawOffersFailed(
                        Gson().fromJson(
                            errorBean.toString(),
                            CommonMessageBean::class.java
                        )
